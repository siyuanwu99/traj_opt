...
namespace ego_planner
{
  ...
  bool PolyTrajOptimizer::ecbsOptTraj(const std::vector<Eigen::Matrix<double, 3, 3>> &iniStates, const std::vector<Eigen::Matrix<double, 3, 3>> &finStates,
                          const std::vector<Eigen::MatrixXd> &initInnerPtss, const std::vector<Eigen::VectorXd> &initTs,
                          std::vector<Eigen::MatrixXd> &optimal_points, double &final_cost, const std::vector<int> &closeSet)
  {
    closeSet_ = closeSet;

    for(int i = 0; i < central_num_; i++){
      if (initInnerPtss[i].cols() != (initTs[i].size() - 1))
      {
        ROS_ERROR("initInnerPts.cols() != (initT.size()-1)");
        return false;
      }

      piece_nums_[i] = initTs[i].size();
      jerkOpts_[i].reset(iniStates[i], finStates[i], piece_nums_[i]);
    }
    ...
    // optimal variances - intermediate points and time duartion;
    int total_variable_num = 4 * (std::accumulate(piece_nums_.begin(), piece_nums_.end(), 0) - central_num_) + central_num_;
    double x_init[total_variable_num];
    int offset = 0;
    for(int i = 0; i < central_num_; i++){
      Eigen::Map<Eigen::MatrixXd> P(x_init + offset, 3, piece_nums_[i] - 1);
      P = initInnerPtss[i];
      Eigen::Map<Eigen::VectorXd> Vt(x_init + offset + initInnerPtss[i].size(), initTs[i].size());
      RealT2VirtualT(initTs[i], Vt);
      offset = offset + initInnerPtss[i].size() + initTs[i].size();
    }
    ...
    do
    {
      ...
      int result = lbfgs::lbfgs_optimize(
          ...
          x_init,
          &final_cost,
          PolyTrajOptimizer::ecbsCostFuncCallback,
          NULL,
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        bool flag_many_jerk = true;
        for(int i = 0; i < central_num_; i++){
          std::vector<std::pair<int, int>> segments_nouse;
          if(finelyCheckAndSetConstraintPoints(cpss_[i], segments_nouse, jerkOpts_[i], false) != CHK_RET::OBS_FREE){
            flag_many_jerk = false;
          }
        }

        std::cout << "flag_many_jerk: " << flag_many_jerk << std::endl;

        if (min_ellip_dist2_ > pow((swarm_clearance_ * 1.25), 2) && flag_many_jerk)
        {
          flag_success = true;
          printf("\033[32mcentral iter=%d,time(ms)=%5.3f,total_t(ms)=%5.3f,cost=%5.3f\n\033[0m", iter_num_, time_ms, total_time_ms, final_cost);
        }
        else
        {
          ...
          flag_still_occ = true;
          restart_nums++;
          printf("\033[32mcentral iter=%d,time(ms)=%5.3f, central fine check collided, keep optimizing\n\033[0m", iter_num_, time_ms);
        }
      }
      else if (result == lbfgs::LBFGSERR_CANCELED)
      {
        ...
        rebound_times++;
        cout << "central iter=" << iter_num_ << ",time(ms)=" << time_ms << ",rebound." << endl;
      }
      else
      {
        cout << "central iter=" << iter_num_ << ",time(ms)=" << time_ms << ",error." << endl;
        ROS_WARN("Central Solver error. Return = %d, %s. Skip this planning.", result, lbfgs::lbfgs_strerror(result));
      }
    ...
    for(int i = 0; i < central_num_; i++){
      optimal_points[i] = cpss_[i].points;
    }
    ...
    do
    {
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        if (min_ellip_dist2_ > pow((swarm_clearance_ * 1.25), 2) &&
            finelyCheckAndSetConstraintPoints(cps_, segments_nouse, jerkOpt_, false) == CHK_RET::OBS_FREE)
        {
      ...
      int id_cps_end, PtsChk_t &pts_check)
  {
    ...
    while (true)
    {
      ...
      if (t < 1e-5 || (pt - pt_last).cwiseAbs().maxCoeff() > RES_2)
      {
      ...
      const bool flag_first_init /*= true*/)
  {
    ...
    if (flag_first_init)
    {
      cps.resize_cp(init_points.cols());
    ...
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
          ...
          if (length > 1e-5)
          {
            ...
            for (double a = length; a >= 0.0; a -= grid_map_->getResolution())
            {
              ...
              if (occ || a < grid_map_->getResolution())
              {
                ...
                cps.base_point[j].push_back((a / length) * intersection_point + (1 - a / length) * init_points.col(j));
                cps.direction[j].push_back((intersection_point - init_points.col(j)).normalized());
                break;
              }
            }
          }
      ...
      if (segment_ids[i].second - segment_ids[i].first == 1)
      {
        ...
        while (true)
        {
          ...
          if (val * init_val <= 0 && (abs(val) > 0 || abs(init_val) > 0)) // val = init_val = 0.0 is not allowed
          {
            ...
            {
              cps.flag_temp[segment_ids[i].first] = true;
              cps.base_point[segment_ids[i].first].push_back(init_points.col(segment_ids[i].first));
              cps.direction[segment_ids[i].first].push_back((intersection_point - middle_point).normalized());
      ...
      if (got_intersection_id >= 0)
      {
        for (int j = got_intersection_id + 1; j <= adjusted_segment_ids[i].second; ++j)
          if (!cps.flag_temp[j])
          {
            cps.base_point[j].push_back(cps.base_point[j - 1].back());
            cps.direction[j].push_back(cps.direction[j - 1].back());
          }
  ...
  bool PolyTrajOptimizer::roughlyCheckConstraintPoints(ConstraintPoints &cps)
  {
    ...
    int i_end = ConstraintPoints::two_thirds_id(cps.points, touch_goal_); // only check closed 2/3 points.
    for (int i = 1; i <= i_end; ++i)
    {

      bool occ = grid_map_->getInflateOccupancy(cps.points.col(i));

      /*** check if the new collision will be valid ***/
      if (occ)
      {
        for (size_t k = 0; k < cps.direction[i].size(); ++k)
        {
          if ((cps.points.col(i) - cps.base_point[i][k]).dot(cps.direction[i][k]) < 1 * grid_map_->getResolution()) // current point is outside all the collision_points.
          {
            occ = false;
            break;
          }
      ...
      if (occ)
      {
        ...
        for (j = i - 1; j >= 0; --j)
        {
          occ = grid_map_->getInflateOccupancy(cps.points.col(j));
          if (!occ)
          {
            in_id = j;
            break;
        ...
        if (j >= cps.cp_size) // fail to get the obs free point
        {
    ...
    if (flag_new_obs_valid)
    {
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        ...
        for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
        {
          ...
          int Astar_id = a_star_pathes[i].size() / 2, last_Astar_id; // Let "Astar_id = id_of_the_most_far_away_Astar_point" will be better, but it needs more computation
          double val = (a_star_pathes[i][Astar_id] - cps.points.col(j)).dot(ctrl_pts_law), init_val = val;
          while (true)
          {
            ...
            val = (a_star_pathes[i][Astar_id] - cps.points.col(j)).dot(ctrl_pts_law);
            ...
            if (val * init_val <= 0 && (abs(val) > 0 || abs(init_val) > 0)) // val = init_val = 0.0 is not allowed
            {
              intersection_point =
                  a_star_pathes[i][Astar_id] +
                  ((a_star_pathes[i][Astar_id] - a_star_pathes[i][last_Astar_id]) *
                   (ctrl_pts_law.dot(cps.points.col(j) - a_star_pathes[i][Astar_id]) / ctrl_pts_law.dot(a_star_pathes[i][Astar_id] - a_star_pathes[i][last_Astar_id])) // = t
                  );
          ...
          if (got_intersection_id >= 0)
          {
            ...
            if (length > 1e-5)
            {
              ...
              for (double a = length; a >= 0.0; a -= grid_map_->getResolution())
              {
                bool occ = grid_map_->getInflateOccupancy((a / length) * intersection_point + (1 - a / length) * cps.points.col(j));

                if (occ || a < grid_map_->getResolution())
                {
                  if (occ)
                    a += grid_map_->getResolution();
                  cps.base_point[j].push_back((a / length) * intersection_point + (1 - a / length) * cps.points.col(j));
                  cps.direction[j].push_back((intersection_point - cps.points.col(j)).normalized());
                  break;
                }
              }
            }
  ...
  double PolyTrajOptimizer::ecbsCostFuncCallback(void *func_data, const double *x, double *grad, const int n)
  {
    ...
    std::vector<Eigen::VectorXd> Ts;
    std::vector<Eigen::MatrixXd> gradPs;
    std::vector<Eigen::VectorXd> gradts;
    std::vector<Eigen::VectorXd> gradTs;

    Ps.resize(opt->central_num_);
    ts.resize(opt->central_num_);
    Ts.resize(opt->central_num_);
    gradPs.resize(opt->central_num_);
    gradts.resize(opt->central_num_);
    gradTs.resize(opt->central_num_);

    double total_cost = 0;
    poly_traj::Trajectory iter_traj;
    int offset = 0;

    for (int i = 0; i < opt->central_num_; i++)
    {
      Eigen::Map<const Eigen::MatrixXd> P(x + offset, 3, opt->piece_nums_[i] - 1);
      Eigen::Map<const Eigen::VectorXd> t(x + offset + (3 * (opt->piece_nums_[i] - 1)), opt->piece_nums_[i]);
      Eigen::VectorXd T(opt->piece_nums_[i]);
      ...
      ts[i] = t;
      Ts[i] = T;
      gradPs[i] = Eigen::MatrixXd::Zero(3, opt->piece_nums_[i] - 1);
      gradTs[i] = Eigen::VectorXd::Zero(opt->piece_nums_[i]);
      gradts[i] = Eigen::VectorXd::Zero(opt->piece_nums_[i]);

      opt->jerkOpts_[i].generate(P, T);
      // update swarm traj
      iter_traj = opt->jerkOpts_[i].getTraj();
      opt->swarm_trajs_->at(opt->closeSet_[i]).traj = iter_traj;
      opt->swarm_trajs_->at(opt->closeSet_[i]).duration = iter_traj.getTotalDuration();

      offset = offset + 3 * (opt->piece_nums_[i] - 1) + opt->piece_nums_[i];
    ...
    for (int i = 0; i < opt->central_num_; i++)
    {
      ...
      opt->initAndGetSmoothnessGradCost2PT(opt->jerkOpts_[i], gradTs[i], smoo_cost); // Smoothness cost

      opt->cenAddPVAGradCost2CT(i, Ts, opt->cpss_[i], gradTs, obs_swarm_feas_qvar_costs, opt->cps_num_prePiece_); // Time int cost

      if (opt->iter_num_ > 3 && smoo_cost / opt->piece_nums_[i] < 10.0) // 10.0 is an experimental value that indicates the trajectory is smooth enough.
      {
        opt->roughlyCheckConstraintPoints(opt->cpss_[i]);
      }

      opt->jerkOpts_[i].getGrad2TP(gradTs[i], gradPs[i]);

      opt->VirtualTGradCost(Ts[i], ts[i], gradTs[i], gradts[i], time_cost);

      total_cost += (smoo_cost + obs_swarm_feas_qvar_costs.sum() + time_cost);

    }

    vector<int> t_idx; // record t index in  grad;
    offset = 0;
    for (int i = 0; i < opt->central_num_; i++)
    {
      Eigen::Map<Eigen::MatrixXd> gradP(grad + offset, 3, opt->piece_nums_[i] - 1);
      Eigen::Map<Eigen::VectorXd> gradt(grad + offset + (3 * (opt->piece_nums_[i] - 1)), opt->piece_nums_[i]);

      gradP = gradPs[i];
      gradt = gradts[i];

      for(int i = 0; i < opt->piece_nums_[i]; i++)
      {
        t_idx.push_back(offset + 3 * (opt->piece_nums_[i] - 1) + i);
      }
  ...
  // double PolyTrajOptimizer::computePerturedCost(Eigen::VectorXd &x_var)
  // {
  ...
  //   Ps.resize(central_num_);
  //   ts.resize(central_num_);
  //   Ts.resize(central_num_);
  //   gradPs.resize(central_num_);
  //   gradts.resize(central_num_);
  //   gradTs.resize(central_num_);
  ...
  //   for (int i = 0; i < central_num_; i++)
  //   {
  //     Eigen::Map<const Eigen::MatrixXd> P(x_var.data() + offset, 3, piece_nums_[i] - 1);
  //     Eigen::Map<const Eigen::VectorXd> t(x_var.data() + offset + (3 * (piece_nums_[i] - 1)), piece_nums_[i]);
  //     Eigen::VectorXd T(piece_nums_[i]);
  ...
  //     ts[i] = t;
  //     Ts[i] = T;
  //     gradPs[i] = Eigen::MatrixXd::Zero(3, piece_nums_[i] - 1);
  //     gradTs[i] = Eigen::VectorXd::Zero(piece_nums_[i]);
  //     gradts[i] = Eigen::VectorXd::Zero(piece_nums_[i]);

  //     jerkOpts_[i].generate(P, T);
  //     // update swarm traj
  //     iter_traj = jerkOpts_[i].getTraj();
  //     swarm_trajs_->at(closeSet_[i]).traj = iter_traj;
  //     swarm_trajs_->at(closeSet_[i]).duration = iter_traj.getTotalDuration();

  //     offset = offset + 3 * (piece_nums_[i] - 1) + piece_nums_[i];
  ...
  //     initAndGetSmoothnessGradCost2PT(jerkOpts_[i], gradTs[i], smoo_cost); // Smoothness cost

  //     cenAddPVAGradCost2CT(i, Ts, cpss_[i], gradTs, obs_swarm_feas_qvar_costs, cps_num_prePiece_); // Time int cost

  //     jerkOpts_[i].getGrad2TP(gradTs[i], gradPs[i]);

  //     VirtualTGradCost(Ts[i], ts[i], gradTs[i], gradts[i], time_cost);

  //     // total_cost += (smoo_cost + time_cost);
  //     total_cost += (smoo_cost + obs_swarm_feas_qvar_costs.sum() + time_cost);
  ...
  // double PolyTrajOptimizer::computePerturedSmoothCost(Eigen::VectorXd &x_var, std::vector<poly_traj::MinJerkOpt> &jerkOpts_pert)
  // {
  //   int offset = 0;
  //   std::vector<Eigen::VectorXd> gradTs;
  //   gradTs.resize(central_num_);
  ...
  //     jerkOpts_pert[i].generate(P, T);

  //     offset = offset + 3 * (piece_nums_[i] - 1) + piece_nums_[i];
  //   }

  //   double total_smooth_cost_perturbed = 0;


  //   for (int i = 0; i < central_num_; i++)
  //   {
  //     double smoo_cost = 0;
  //     initAndGetSmoothnessGradCost2PT(jerkOpts_pert[i], gradTs[i], smoo_cost);
  //     total_smooth_cost_perturbed += smoo_cost;
  //   }

  //   return total_smooth_cost_perturbed;

  // }

  template <typename EIGENVEC>
  void PolyTrajOptimizer::cenAddPVAGradCost2CT(int id, std::vector<Eigen::VectorXd> &Ts, ConstraintPoints &cps, std::vector<EIGENVEC> &gradTs, Eigen::VectorXd &costs, const int &K)
  {
    int N = gradTs[id].size(); //central idx
    ...
    for (int i = 0; i < N; ++i)
    {
      const Eigen::Matrix<double, 6, 3> &c = jerkOpts_[id].get_b().block<6, 3>(i * 6, 0);
      step = jerkOpts_[id].get_T1()(i) / K;
      s1 = 0.0;
      // innerLoop = K;
      ...
      for (int j = 0; j <= K; ++j)
      {
        ...
        cps.points.col(i_dp) = pos;

        // collision
        if (obstacleGradCostP(cps, i_dp, pos, gradp, costp))
        {
          gradViolaPc = beta0 * gradp.transpose();
          gradViolaPt = alpha * gradp.transpose() * vel;
          jerkOpts_[id].get_gdC().block<6, 3>(i * 6, 0) += omg * step * gradViolaPc;
          gradTs[id](i) += omg * (costp / K + step * gradViolaPt);
          costs(0) += omg * step * costp;
        }

        // swarm
        double gradt, grad_prev_t;
        if (cenSwarmGradCostP(id, omg, step, gradTs, Ts, cps, i_dp, t + step * j, pos, vel, gradp, gradt, grad_prev_t, costp))
        {
          gradViolaPc = beta0 * gradp.transpose();
          ...
          if (i > 0)
          {
            gradTs[id].head(i).array() += omg * step * grad_prev_t;// 0 ~ i-1
          }
        ...
        if (feasibilityGradCostV(vel, gradv, costv))
        {
          ...
          jerkOpts_[id].get_gdC().block<6, 3>(i * 6, 0) += omg * step * gradViolaVc;
          gradTs[id](i) += omg * (costv / K + step * gradViolaVt);
        ...
        if (feasibilityGradCostA(acc, grada, costa))
        {
          ...
          jerkOpts_[id].get_gdC().block<6, 3>(i * 6, 0) += omg * step * gradViolaAc;
          gradTs[id](i) += omg * (costa / K + step * gradViolaAt);
    ...
    distanceSqrVarianceWithGradCost2p(cps.points, gdp, var);
    // cout << "var=" << var << endl;
    ...
    for (int i = 0; i < N; ++i)
    {
      ...
      for (int j = 0; j <= K; ++j)
      {
        ...
        beta1 << 0.0, 1.0, 2.0 * s1, 3.0 * s2, 4.0 * s3, 5.0 * s4;
        alpha = 1.0 / K * j;
        vel = jerkOpts_[id].get_b().block<6, 3>(i * 6, 0).transpose() * beta1;

        omg = (j == 0 || j == K) ? 0.5 : 1.0;

        gradViolaPc = beta0 * gdp.col(i_dp).transpose();
        gradViolaPt = alpha * gdp.col(i_dp).transpose() * vel;
        jerkOpts_[id].get_gdC().block<6, 3>(i * 6, 0) += omg * gradViolaPc;
        gradTs[id](i) += omg * (gradViolaPt);
  ...
  bool PolyTrajOptimizer::cenSwarmGradCostP(int i_cur, 
                                         double &omg,
                                         double &step,
                                         std::vector<Eigen::VectorXd> &gradTs,
                                         std::vector<Eigen::VectorXd> &Ts,
                                         ...
                                         double &costp)
  {
    if (i_dp <= 0 || i_dp > ConstraintPoints::two_thirds_id(cps.points, touch_goal_)) // only apply to first 2/3
      return false;
    ...
    for (int id = 0; id < (int)swarm_trajs_->size(); id++)
    {

      if ((swarm_trajs_->at(id).drone_id < 0) || (id == closeSet_[i_cur]))
      // if ((swarm_trajs_->at(id).drone_id < 0) || (id == closeSet_[i_cur]) || 
      //   ((id < closeSet_[i_cur]) && (std::find(closeSet_.begin(), closeSet_.end(), id) != closeSet_.end())))
      // if ((swarm_trajs_->at(id).drone_id < 0) || (id == closeSet_[i_cur]) || 
      //   (std::find(closeSet_.begin(), closeSet_.end(), id) != closeSet_.end()))
      {
        continue;
      }

      double traj_i_satrt_time = swarm_trajs_->at(id).start_time;
      double pt_time = (swarm_trajs_->at(closeSet_[i_cur]).start_time - traj_i_satrt_time) + t;
      ...
      if (dist2_err3 > 0)
      {
        ...
        // if((id > closeSet_[i_cur]) && (std::find(closeSet_.begin(), closeSet_.end(), id) != closeSet_.end()))
        if(std::find(closeSet_.begin(), closeSet_.end(), id) != closeSet_.end())
        {
          Eigen::Vector3d dJ_dP_other = wei_swarm_ * 3 * dist2_err2 * (2) * Eigen::Vector3d(inv_b2 * dist_vec(0), inv_b2 * dist_vec(1), inv_a2 * dist_vec(2));
          double gradt_other = dJ_dP_other.dot(swarm_v);

          // id central idx
          std::vector<int>::iterator it = std::find(closeSet_.begin(), closeSet_.end(), id);
          int cen_idx = std::distance(closeSet_.begin(), it);

          int t_idx = 0;
          double t_sum = 0;
          for(int i = 0; i < Ts[cen_idx].size(); i++){
            t_sum += Ts[cen_idx](i);
            if(t < t_sum){
              t_idx = i;
              break;
          ...
          if(t < t_sum){
            // ROS_WARN("central avoidance --- add gdc to other drone!");
            double t_other, t_other2, t_other3, t_other4, t_other5;
            t_other = t - t_sum + Ts[cen_idx](t_idx);
            t_other2 = t_other * t_other;
            t_other3 = t_other2 * t_other;
            t_other4 = t_other2 * t_other2;
            t_other5 = t_other4 * t_other;

            Eigen::Matrix<double, 6, 1> beta0_other;
            beta0_other << 1.0, t_other, t_other2, t_other3, t_other4, t_other5;

            jerkOpts_[cen_idx].get_gdC().block<6, 3>(t_idx * 6, 0) += omg * step * beta0_other * dJ_dP_other.transpose();
          }
          else{
            ROS_WARN("over-border! uniform speed.");
            t_idx = Ts[cen_idx].size();
          }
          
          if(t_idx > 0){
            gradTs[cen_idx].head(t_idx).array() += omg * step * gradt_other * (-1);
          }
        }
  ...
  double PolyTrajOptimizer::costFunctionCallback(void *func_data, const double *x, double *grad, const int n)
  {
    ...
    opt->initAndGetSmoothnessGradCost2PT(opt->jerkOpt_, gradT, smoo_cost); // Smoothness cost

    opt->addPVAGradCost2CT(opt->jerkOpt_, opt->cps_, gradT, obs_swarm_feas_qvar_costs, opt->cps_num_prePiece_); // Time int cost
    ...
    {
      opt->roughlyCheckConstraintPoints(opt->cps_);
    }
  ...
  template <typename EIGENVEC>
  void PolyTrajOptimizer::initAndGetSmoothnessGradCost2PT(poly_traj::MinJerkOpt &jerkOpt, EIGENVEC &gdT, double &cost)
  {
    jerkOpt.initGradCost(gdT, cost);
  }

  template <typename EIGENVEC>
  void PolyTrajOptimizer::addPVAGradCost2CT(poly_traj::MinJerkOpt &jerkOpt, ConstraintPoints &cps, EIGENVEC &gdT, Eigen::VectorXd &costs, const int &K)
  {
    int N = gdT.size();
    Eigen::Vector3d pos, vel, acc, jer;
    Eigen::Vector3d gradp, gradv, grada;
    ...
    for (int i = 0; i < N; ++i)
    {
      ...
      const Eigen::Matrix<double, 6, 3> &c = jerkOpt.get_b().block<6, 3>(i * 6, 0);
      step = jerkOpt.get_T1()(i) / K;
      s1 = 0.0;
      // innerLoop = K;
      ...
      for (int j = 0; j <= K; ++j)
      {
        ...
        if (obstacleGradCostP(cps, i_dp, pos, gradp, costp))
        {
          gradViolaPc = beta0 * gradp.transpose();
          gradViolaPt = alpha * gradp.transpose() * vel;
          jerkOpt.get_gdC().block<6, 3>(i * 6, 0) += omg * step * gradViolaPc;
        ...
        // swarm
        double gradt, grad_prev_t;
        if (swarmGradCostP(cps, i_dp, t + step * j, pos, vel, gradp, gradt, grad_prev_t, costp))
        {
          gradViolaPc = beta0 * gradp.transpose();
          gradViolaPt = alpha * gradt;
          jerkOpt.get_gdC().block<6, 3>(i * 6, 0) += omg * step * gradViolaPc;
          ...
          if (i > 0)
          {
            gdT.head(i).array() += omg * step * grad_prev_t;// 0 ~ i-1
          }
          costs(1) += omg * step * costp;
        }
        ...
        if (feasibilityGradCostV(vel, gradv, costv))
        {
          gradViolaVc = beta1 * gradv.transpose();
          gradViolaVt = alpha * gradv.transpose() * acc;
          jerkOpt.get_gdC().block<6, 3>(i * 6, 0) += omg * step * gradViolaVc;
        ...
        if (feasibilityGradCostA(acc, grada, costa))
        {
          gradViolaAc = beta2 * grada.transpose();
          gradViolaAt = alpha * grada.transpose() * jer;
          jerkOpt.get_gdC().block<6, 3>(i * 6, 0) += omg * step * gradViolaAc;
    ...
    for (int i = 0; i < N; ++i)
    {
      ...
      for (int j = 0; j <= K; ++j)
      {
        ...
        beta1 << 0.0, 1.0, 2.0 * s1, 3.0 * s2, 4.0 * s3, 5.0 * s4;
        alpha = 1.0 / K * j;
        vel = jerkOpt.get_b().block<6, 3>(i * 6, 0).transpose() * beta1;

        omg = (j == 0 || j == K) ? 0.5 : 1.0;

        gradViolaPc = beta0 * gdp.col(i_dp).transpose();
        gradViolaPt = alpha * gdp.col(i_dp).transpose() * vel;
        jerkOpt.get_gdC().block<6, 3>(i * 6, 0) += omg * gradViolaPc;
  ...
  bool PolyTrajOptimizer::obstacleGradCostP(ConstraintPoints &cps, 
                                            const int i_dp,
                                            const Eigen::Vector3d &p,
                                            Eigen::Vector3d &gradp,
                                            double &costp)
  {
    ...
    // Obatacle cost
    for (size_t j = 0; j < cps.direction[i_dp].size(); ++j)
    {
      Eigen::Vector3d ray = (p - cps.base_point[i_dp][j]);
      double dist = ray.dot(cps.direction[i_dp][j]);
      double dist_err = obs_clearance_ - dist;
      double dist_err_soft = obs_clearance_soft_ - dist;
      Eigen::Vector3d dist_grad = cps.direction[i_dp][j];
  ...
  bool PolyTrajOptimizer::swarmGradCostP(ConstraintPoints &cps,
                                          const int i_dp,
                                         const double t,
                                         const Eigen::Vector3d &p,
                                         const Eigen::Vector3d &v,
                                         ...
                                         double &costp)
  {
    ...
    for (size_t id = 0; id < swarm_trajs_->size(); id++)
    {
      ...
      double traj_i_satrt_time = swarm_trajs_->at(id).start_time;
      double pt_time = (t_now_ - traj_i_satrt_time) + t; // never assign a high-precision global time to a double directly!
