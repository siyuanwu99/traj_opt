...
#include "polynomial/gcopter.hpp"
#include "geometry/firi.hpp"
#include "geometry/sfc_gen.hpp"
#include "polynomial/trajectory.hpp"
#include "misc/voxel_map.hpp"
 ...
 public:
  ...
  inline void plan() {
    if (startGoal.size() == 2) {
      ...
      std::cout << "after shortcut: hPolys.size() = " << hPolys.size() << std::endl;
      for (size_t i = 0; i < hPolys.size(); i++) {
        std::cout << "hPolys[" << i << "].rows() = " << hPolys[i].rows() << std::endl;
      }
  ...
  inline void process() {
    ...
    flatmap.reset(physicalParams(0), physicalParams(1), physicalParams(2), physicalParams(3),
                  physicalParams(4), physicalParams(5));
    ...
    if (traj.getPieceNum() > 0) {
      ...
      if (delta > 0.0 && delta < traj.getTotalDuration()) {
        ...
        flatmap.forward(traj.getVel(delta), traj.getAcc(delta), traj.getJer(delta), 0.0, 0.0, thr,
                        quat, omg);
